...
exchanges and finds price spreads between them that could be arbitrage opportunities. The idea is to fetch prices every
N seconds, find the largest spreads, then subscribe to pairs via WebSockets to find entry opportunities. Then repeat
after M seconds. I'd love to subscribe to **all** pairs via WebSockets, but rate limits... I have a working
production-grade app like this written in NestJS, but I wanted to see if I can achieve something similar in Rust and how
it would look.
...
- **KuCoin stopped working.** I think they'd changed API.
